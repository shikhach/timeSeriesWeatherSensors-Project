#Phase1, Phase2 Model2, Phase2 Model1
...
P1 <- read.table("phase2/Temperr/P1.csv", sep=",", header=TRUE)
P2M1 <- read.table("phase2/Temperr/P2M1.csv", sep=",", header=TRUE)
P2M2 <- read.table("phase2/Temperr/P2M2.csv", sep=",", header=TRUE)

budgetList = c(0,5,10,20,25)

for(i in 1:2) {
  for(j in 1:length(budgetList)) {
    errWin = c(P1[j,(2*i)],P2M1[j,(2*i)],P2M2[j,(2*i)])
    errVar = c(P1[j,(2*i)+1],P2M1[j,(2*i)+1],P2M2[j,(2*i)+1])
    plotName = "Data : "
    if(i==1) {
    ...
    barplot(rbind(errWin,errVar), main=plotName, names.arg=c("Ph1","Ph2_Hr","P2_Day"),col=c("darkblue","red"),
